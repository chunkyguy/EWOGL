...
uniform lowp sampler2DShadow us2s_Tex0;
...
void main()
{
  ...
  lowp float lf1k_Shadow = textureProj(us2s_Tex0, vv4s_Texcoord);
  lowp vec4 lv4k_Final = vec4((material.uv4k_Color.xyz * lf1k_Diffuse + light.uv4k_Color.xyz * lf1k_Specular) * lf1k_Shadow, 1.0);
  
  //  FragColor = vec4(light.uv4e_Position.xyz, 1.0);
  //FragColor = vec4(vec3(lf1k_Shadow), 1.0);
  //FragColor = vec4(vv3e_Normal, 1.0);
  //FragColor = vv4s_Texcoord;
  //FragColor = vec4(lf1k_Shadow);

...
Mesh *CreateMeshFromFile(Mesh *mesh, const char *filename) {
 ...
 BundlePath(path_buffer, filename);
...
const Mesh *RenderMesh(const Mesh *mesh,   /*	TGLK mesh to be rendered */
                       ...
                       const Frustum *frustum, /* TGLK perpective to be applied*/
                       const Vec4f *color
                       ) {
 /* Matrices used */
 Mat4 mvMat;
 ...
 /*	Bind tGLK data to tGLK associated uniform variable in tGLK shader
  First gets tGLK location of that variable in tGLK shader using its name
  TGLKn passes tGLK matrix to that variable
  */
 ...
 /*
  ...
  For indexed geometry, use tGLK function glDrawElements() with an index list.
